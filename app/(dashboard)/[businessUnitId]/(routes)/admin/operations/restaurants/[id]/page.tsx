...
const EditRestaurantPage: React.FC = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (restaurantData) {
          ...
          setFormData({
            ...
            averageMeal: restaurantData.averageMeal ? parseFloat(restaurantData.averageMeal) : null,
            ...
            // FIX: Added missing fields from the form
            totalSeats: restaurantData.totalSeats,
            privateRooms: restaurantData.privateRooms,
            outdoorSeating: restaurantData.outdoorSeating,
            airConditioned: restaurantData.airConditioned,
            acceptsReservations: restaurantData.acceptsReservations,
            advanceBookingDays: restaurantData.advanceBookingDays,
            minPartySize: restaurantData.minPartySize,
            maxPartySize: restaurantData.maxPartySize,
            virtualTourUrl: restaurantData.virtualTourUrl,
            hasMenu: restaurantData.hasMenu,
            menuUrl: restaurantData.menuUrl,
            metaTitle: restaurantData.metaTitle,
            metaDescription: restaurantData.metaDescription,
            dressCode: restaurantData.dressCode || '',
          });

          // Initialize existing restaurant images with proper ID tracking
          if (restaurantData.images && restaurantData.images.length > 0) {
            const existingImages = restaurantData.images.map(img => ({
              fileName: img.image.originalUrl.split('/').pop() || 'image',
              name: img.image.title || img.image.altText || 'Restaurant Image',
    ...
    if (restaurantId && businessUnitId) {
      loadData();
    }
  }, [restaurantId, router, businessUnitId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Filter out new images (ones that weren't in the original restaurant)
      ...
      const restaurantData: UpdateRestaurantData = {
        ...
        averageMeal: formData.averageMeal,
        ...
        restaurantImages: newImages.length > 0 ? newImages : undefined,
