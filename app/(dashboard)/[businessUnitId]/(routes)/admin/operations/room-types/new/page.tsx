...
import { createRoomType, CreateRoomTypeData } from '@/lib/actions/room-type-management';
...
const NewRoomTypePage: React.FC = () => {
  ...
  const handleInputChange = (field: keyof RoomTypeFormData, value: string | number | boolean | string[]) => {
    setFormData(prev => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const roomTypeData: CreateRoomTypeData = {
        ...formData,
        description: formData.description || null,
        bedConfiguration: formData.bedConfiguration || null,
        extraPersonRate: formData.extraPersonRate,
        extraChildRate: formData.extraChildRate,
        floorPlan: formData.floorPlan || null,
        roomTypeImages: images.images.length > 0 ? images.images : undefined,
      };

      const result = await createRoomType(roomTypeData);
      ...
      if (result.success) {
        ...
        setSnackbar({
          open: true,
          message: result.message || 'Failed to create room type',
          severity: 'error',
        });
      }
    } catch (error) {
      setSnackbar({
        open: true,
        message: `An error occurred while creating room type: ${error}`,
        severity: 'error',
      });
    } finally {
      setLoading(false);
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                    ...
                    <TextField
                      ...
                      onChange={(e) => handleInputChange('roomSize', e.target.value ? parseFloat(e.target.value) : "")}
                      sx={{
                        flex: 1,
                        minWidth: 150,
                        '& .MuiOutlinedInput-root': {
              ...
              <CardContent sx={{ p: 4 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                  ...
                  <TextField
                    ...
                    onChange={(e) => handleInputChange('extraPersonRate', e.target.value ? parseFloat(e.target.value) : "")}
                    sx={{
                      flex: 1,
                      minWidth: 200,
                      '& .MuiOutlinedInput-root': {
                  ...
                  <TextField
                    ...
                    onChange={(e) => handleInputChange('extraChildRate', e.target.value ? parseFloat(e.target.value) : "")}
                    sx={{
                      flex: 1,
                      minWidth: 200,
                      '& .MuiOutlinedInput-root': {
