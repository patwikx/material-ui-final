...
import { getRoomRateById, updateRoomRate, UpdateRoomRateData, RoomRateData } from '@/lib/actions/room-rates-management';
...
const EditRoomRatePage: React.FC = () => {
  const router = useRouter();
  const params = useParams();
  const roomRateId = params.id as string;
  const { businessUnitId } = useBusinessUnit();

  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [roomRate, setRoomRate] = useState<RoomRateData | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [roomRateData, types] = await Promise.all([
          getRoomRateById(roomRateId),
          getRoomTypes()
        ]);
        ...
        if (roomRateData) {
          setRoomRate(roomRateData);
          setFormData({
            roomTypeId: roomRateData.roomTypeId,
            name: roomRateData.name,
            description: roomRateData.description || '',
            baseRate: parseFloat(roomRateData.baseRate),
            currency: roomRateData.currency,
            validFrom: roomRateData.validFrom ? new Date(roomRateData.validFrom).toISOString().slice(0, 10) : '',
            validTo: roomRateData.validTo ? new Date(roomRateData.validTo).toISOString().slice(0, 10) : '',
            isActive: roomRateData.isActive,
            isDefault: roomRateData.isDefault,
            minStay: roomRateData.minStay,
            maxStay: roomRateData.maxStay,
            minAdvance: roomRateData.minAdvance,
            maxAdvance: roomRateData.maxAdvance,
            monday: roomRateData.monday,
            tuesday: roomRateData.tuesday,
            wednesday: roomRateData.wednesday,
            thursday: roomRateData.thursday,
            friday: roomRateData.friday,
            saturday: roomRateData.saturday,
            sunday: roomRateData.sunday,
          });
        } else {
          setSnackbar({
            open: true,
        ...
        setSnackbar({
          open: true,
          message: `Failed to load room rate: ${error}`,
          severity: 'error',
        });
      } finally {
        setLoading(false);
    ...
    if (roomRateId && businessUnitId) {
      loadData();
    }
  }, [roomRateId, router, businessUnitId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const roomRateData: UpdateRoomRateData = {
        id: roomRateId,
      ...
      const result = await updateRoomRate(roomRateData);
      ...
      if (result.success) {
        ...
        setSnackbar({
          open: true,
          message: result.message || 'Failed to update room rate',
          severity: 'error',
        });
      }
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
      ...
      setSnackbar({
        open: true,
        message: 'An error occurred while updating room rate',
        severity: 'error',
      });
    } finally {
      setSaving(false);
    ...
    >
      <Container maxWidth="xl" sx={{ py: 4 }}>
        <Box sx={{ mb: 6 }}>
          ...
          >
            Update room rate information and settings
          </Typography>
        </Box>
...
export default EditRoomRatePage;